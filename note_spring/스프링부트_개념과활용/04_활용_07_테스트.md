...
* 실제 서블릿이 랜덤한 포트에 띄워진다.
  * 테스트하면 `Service` 단까지 들어간다. 

* MockMvc를 사용하지 않고, `RestTemplate`이나 `테스트용 RestTemplate`  , `테스트용 Web Client`를 써야한다.



#### 1. 테스트용 RestTemplate 사용하기
...
`@MockBean` 어노테이션으로 테스트하려는 컨트롤러와 연관관계있는 mockSampleController로 만들면, ApplicationContext에 들어있는 빈을 Mock으로 만든 객체로 교체 한다.

실질적으로 컨트롤러는 Mock SampleService를 쓰게 된다. (원본이 아니라)
